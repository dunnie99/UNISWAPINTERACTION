...
//   const amountToSwap1 = await ethers.utils.parseEther("90");
//   console.log(amountToSwap1);

//   const amountToReceive1 = await ethers.utils.parseEther("100");
//   console.log(amountToReceive1);
 ...
 //Impersonation
  ...
  await DaiContract.connect(impersonatedSigner).approve(ROUTER, 100000);

  await Uniswap.connect(impersonatedSigner).swapExactTokensForETH(
    100000,
    10,
    path3,
    DAIHolder,
  ...
  const ETHbalanceAfter1 = await ethers.provider.getBalance(DAIHolder);
  console.log(`ETHBalance1 after ${ETHbalanceAfter1}`);
  const holderBalanceAftertoken = await DaiContract.balanceOf(DAIHolder);
  console.log(`Daibalance1 After token${holderBalanceAftertoken}`);
